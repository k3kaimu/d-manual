...
その関数が、メモリ的に安全かどうかについて、属性をつけることができます。
...
+ <b>セーフ関数</b>`@safe`
    ...
    [Functions#safe-functions - D Programming Language](http://dlang.org/function.html#safe-functions)</small>  


    セーフ関数はコンパイル時に解析され、セーフ関数であるのにメモリセーフでない操作をしている場合には、コンパイルエラーとなります。
...
+ <b>信頼済み関数</b>`@trusted`
    ...
    {
        return arr.ptr[arr.length - 1];            // *(p + 1)なので、fooはセーフ関数になれない
