...
ここからは、さっき書いたHello Worldプログラムを解析していきます。
...
というわけで、`std.stdio`というのは`std`パッケージに属する`stdio`モジュールという意味です。
...
つまり、「`foo`というのを使いたい！`foo`は`bar`モジュールで定義されてる」という場合には、`foo`を使う前に`import bar;`と書かなければいけません。
