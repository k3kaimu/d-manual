...
これは、UFCS(Uniform Function Call Syntax)といい、「関数が引数を1つだけ取る場合に`fun(arg)`じゃなくて`arg.fun()`及び`arg.fun`と書ける」記法です。
以前は、`arg`が配列(`string`は配列と言いましたね)の場合だけに許された記法でしたが、dmd 2.059でどのような型でも可能になりました。
